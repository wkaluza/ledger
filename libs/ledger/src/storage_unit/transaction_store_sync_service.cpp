        ...
        "The number of peers the sync can use")}
{
  ...
  state_machine_->RegisterHandler(
      State::QUERY_OBJECT_COUNTS, this, &TransactionStoreSyncService::OnQueryObjectCounts);
  state_machine_->RegisterHandler(
      State::RESOLVING_OBJECT_COUNTS, this, &TransactionStoreSyncService::OnResolvingObjectCounts);
  state_machine_->RegisterHandler(
      State::QUERY_SUBTREE, this, &TransactionStoreSyncService::OnQuerySubtree);
  state_machine_->RegisterHandler(
      State::RESOLVING_SUBTREE, this, &TransactionStoreSyncService::OnResolvingSubtree);
  state_machine_->RegisterHandler(
      State::QUERY_OBJECTS, this, &TransactionStoreSyncService::OnQueryObjects);
  state_machine_->RegisterHandler(
      State::RESOLVING_OBJECTS, this, &TransactionStoreSyncService::OnResolvingObjects);
  state_machine_->RegisterHandler(
      State::TRIM_CACHE, this, &TransactionStoreSyncService::OnTrimCache);
...
TransactionStoreSyncService::State TransactionStoreSyncService::OnResolvingObjectCounts()
{
  ...
  {
    FETCH_LOG_ERROR(
        LOGGING_NAME, "Lane ", cfg_.lane_id, ": ", "Failed object count promises: ", counts.failed);
  ...
  if (max_object_count_ == 0)
  {
    FETCH_LOG_INFO(
        LOGGING_NAME,
        "Network appears to have no transactions! Number of peers: ",
...
TransactionStoreSyncService::State TransactionStoreSyncService::OnQuerySubtree()
{
  ...
  for (auto const &connection : directly_connected_peers)
  {
    ...
    *reinterpret_cast<decltype(root) *>(transactions_prefix.char_pointer()) = root;
    ...
    auto promise = PromiseOfTxList(client_->CallSpecificAddress(
        connection,
        RPC_TX_STORE_SYNC,
        TransactionStoreSyncProtocol::PULL_SUBTREE,
        transactions_prefix,
        root_size_));
  ...
  FETCH_LOG_INFO(
      ...
      cfg_.lane_id,
      ": ",
      "QueryingSubtree: requested ",
      orig_num_of_roots - roots_to_sync_.size(),
      " root(s). Remaining roots to sync: ",
      roots_to_sync_.size(),
      " / ",
...
TransactionStoreSyncService::State TransactionStoreSyncService::OnResolvingSubtree()
{
  ...
  {
    FETCH_LOG_WARN(
        LOGGING_NAME, "Lane ", cfg_.lane_id, ": ", "Failed subtree promises count ", counts.failed);
...
TransactionStoreSyncService::State TransactionStoreSyncService::OnQueryObjects()
{
  ...
  for (auto const &connection : muddle_.GetDirectlyConnectedPeers())
  {
    ...
    {
      FETCH_LOG_WARN(
          LOGGING_NAME, "Lane ", cfg_.lane_id, ": Explicitly requesting ", digests.size(), " TXs");

      auto p2 = PromiseOfTxList(client_->CallSpecificAddress(
          connection,
          RPC_TX_STORE_SYNC,
          TransactionStoreSyncProtocol::PULL_SPECIFIC_OBJECTS,
          digests));
      if (!pending_objects_.Add(connection, p2))
      {
        FETCH_LOG_WARN(
            LOGGING_NAME, "Failed to add promise of transactions to queue - call specific");
      }
    }
  }
...
TransactionStoreSyncService::State TransactionStoreSyncService::OnResolvingObjects()
{
  ...
  for (auto &result : pending_objects_.Get(MAX_OBJECT_RESOLUTION_PER_CYCLE))
  {
    ...
    {
      FETCH_LOG_DEBUG(
          LOGGING_NAME, "Lane ", cfg_.lane_id, ": Got ", result.promised.size(), " objects!");
    }
  ...
  {
    FETCH_LOG_DEBUG(
        LOGGING_NAME, "Lane ", cfg_.lane_id, " Synchronised ", synced_tx, " requested txs");
  }
