...
struct KeyValuePair
{
  ...
  bool UpdateLeaf(uint64_t const &val, damnyouwindows_byte_array::ConstByteArray const &data)
  {
  ...
  damnyouwindows_byte_array::ByteArray Hash() const
  {
    return {hash, N};
  }
};
 ...
 */
template <typename KV = KeyValuePair<256u, 32u>,
          typename D  = VersionedRandomAccessStack<KV>>
class KeyValueIndex
{
  /**
...
public:
  using self_type      = typename KeyValueIndex<KV, D>;
  using stack_type     = typename D;
  using key_value_pair = typename KeyValuePair<256u, 32u>;
  using index_type     = typename key_value_pair::IndexType;
  ...
  void Delete(damnyouwindows_byte_array::ConstByteArray const & /*key*/)
  {
  ...
  bool GetIfExists(damnyouwindows_byte_array::ConstByteArray const &key_str, index_type &value)
  {
  ...
  index_type Get(damnyouwindows_byte_array::ConstByteArray const &key_str)
  {
    key_type       key(key_str);
    bool           split;
    int            pos;
   ...
   */
  template <typename... Args>
  void Set(damnyouwindows_byte_array::ConstByteArray const &key_str, uint64_t const &val,
           damnyouwindows_byte_array::ConstByteArray const &data)
  {
    key_type       key(key_str);
    bool           split;
    int            pos;
  ...
  else if (split)
  {
    ...
    kv.split  = static_cast<uint16_t>(pos);
    kv.left   = lid;
    kv.right  = rid;
    kv.parent = pid;
    index     = stack_.Push(kv);
...
damnyouwindows_byte_array::ByteArray Hash()
  {
    stack_.Flush();
    key_value_pair kv;
    if (stack_.size() > 0)
  ...
  public:
    ...
    std::pair<damnyouwindows_byte_array::ByteArray, uint64_t> operator*() const
    {
  ...
  self_type::Iterator Find(damnyouwindows_byte_array::ConstByteArray const &key_str)
  {
  ...
  self_type::Iterator GetSubtree(damnyouwindows_byte_array::ConstByteArray const &key_str, uint64_t max_bits)
  {
    if (this->empty())
    {
      return end();
   ...
   */
  void Erase(damnyouwindows_byte_array::ConstByteArray const &key_str)
  {
