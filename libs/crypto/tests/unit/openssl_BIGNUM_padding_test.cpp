...
protected:
      ...
      bool const                        expected_comparison_result = true)
  {
    ...
    ASSERT_NE(
        nullptr,
        BN_bin2bn(orig_bin_bn.pointer(), static_cast<int>(orig_bin_bn.size()), orig_bn.get()));
    ...
    case ePadding::suffix:
      ASSERT_EQ(
          padding_bin,
          padded_bin_bn.SubArray(padded_bin_bn.size() - padding_bin.size(), padding_bin.size()));
      break;
    };
    ...
    ASSERT_NE(
        nullptr,
        BN_bin2bn(
            static_cast<byte_array::ConstByteArray const &>(padded_bin_bn).pointer(),
            static_cast<int>(padded_bin_bn.size()),
            padded_bn.get()));
