...
public:
  ...
  Setting(
      SettingCollection &reg, std::string name, T const &default_value, std::string description);
...
template <typename T>
Setting<T>::Setting(
    SettingCollection &reg, std::string name, T &&default_value, std::string description)
  : SettingBase{reg, std::move(name), std::move(description)}
