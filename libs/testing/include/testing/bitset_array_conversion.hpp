...
damnyouwindows_byte_array::ByteArray ToByteArray(StdArray<T, SIZE> const &from)
{
  return {reinterpret_cast<damnyouwindows_byte_array::ConstByteArray::container_type const *>(from.data()),
          from.size() * sizeof(typename StdArray<T, SIZE>::value_type)};
}

template <std::size_t BITS>
meta::EnableIf<meta::IsLog2(BITS), damnyouwindows_byte_array::ByteArray> ToByteArray(std::bitset<BITS> const &from)
{
  using T = damnyouwindows_byte_array::ConstByteArray::container_type;
