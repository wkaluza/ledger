    ...
    VectorRegister<fixed_point::fp64_t, 128> const &x)
{
  ...
  ret.data()                       = _mm_blendv_epi8(
      ret.data(),
      VectorRegister<fixed_point::fp64_t, 128>(fixed_point::fp64_t::NaN).data(),
      mask.data());
  return {ret.data()};
}
    ...
    VectorRegister<fixed_point::fp64_t, 256> const &x)
{
  ...
  ret.data()                       = _mm256_blendv_epi8(
      ret.data(),
      VectorRegister<fixed_point::fp64_t, 256>(fixed_point::fp64_t::NaN).data(),
      mask.data());
  return {ret.data()};
}
    ...
    VectorRegister<fixed_point::fp64_t, 128> const &b)
{
  ...
  sum.data() = _mm_blendv_epi8(
      sum.data(),
      VectorRegister<fixed_point::fp64_t, 128>::MaskPosInf().data(),
      mask_pos_inf.data());
  // Replace -inf in the final product using the mask
  sum.data() = _mm_blendv_epi8(
      sum.data(),
      VectorRegister<fixed_point::fp64_t, 128>::MaskNegInf().data(),
      mask_neg_inf.data());
  ...
  bool is_infinity = any_equal_to(
      mask_pos_inf | mask_neg_inf, VectorRegister<fixed_point::fp64_t, 128>::MaskAllBits());
    ...
    VectorRegister<fixed_point::fp64_t, 256> const &b)
{
  ...
  sum.data() = _mm256_blendv_epi8(
      sum.data(),
      VectorRegister<fixed_point::fp64_t, 256>::MaskPosInf().data(),
      mask_pos_inf.data());
  // Replace -inf in the final product using the mask
  sum.data() = _mm256_blendv_epi8(
      sum.data(),
      VectorRegister<fixed_point::fp64_t, 256>::MaskNegInf().data(),
      mask_neg_inf.data());
  // Replace NaN in the final product using the mask
  sum.data() = _mm256_blendv_epi8(
      sum.data(),
  ...
  bool is_infinity = any_equal_to(
      mask_pos_inf | mask_neg_inf, VectorRegister<fixed_point::fp64_t, 256>::MaskAllBits());
